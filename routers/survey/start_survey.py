...
@router.message(Command("survey",prefix="!/"),default_state)#Default state нужен чтобы со старта бота у нас было состояние одно несмотря что мы не устанавливали пока состояние
async def start_survey(message:Message,state:FSMContext):
    await message.answer(f"Hello {message.from_user.full_name}. What is your name?")
    await state.set_state(Survey.name)
    
@router.message(Command("cancel"),StateFilter(Survey(),SportDetails()))#Здесь мы передаем два состояния потому что у нас два класса Survey и SportDetails  
